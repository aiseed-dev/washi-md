...
- **방점·방선**: `[텍스트]{.sesame_dot}` (Pandoc 스타일 클래스 스팬) —
  ...
  가로쓰기·세로쓰기 모두 지원
- **폼(양식)**: 본문 중 `[.form]` 블록(바로 뒤에 JSON 정의)을 확인 화면이 있는
  대화형 폼으로. text/email/tel/textarea/select/radio/checkbox 항목·필수·서식
  (우편번호/전화/가나)·이메일 일치 확인 지원. 폼이 있는 페이지에만 렌더링
  자산을 인라인 (Markdown·AsciiDoc 모두 지원)
