...
- **著重號／著重線**: `[文字]{.sesame_dot}` (Pandoc 風格類別 span) —
  ...
  CSS,直橫排皆支援
- **表單**: 內文中的 `[.form]` 區塊(緊接其後為 JSON 定義)會變成帶確認畫面的
  互動式表單。支援 text/email/tel/textarea/select/radio/checkbox 欄位、必填、
  格式(郵遞區號/電話/假名)、電子郵件一致確認。僅在含表單的頁面內嵌繪製資產
  (Markdown 與 AsciiDoc 皆支援)
